...
# PORTAL_APP="$(pnpm nx show project "$2" --json | jq -r '.root')"
# jq is not installed on the build machine, so use node
PORTAL_APP="$(pnpm nx show project "$2" --json | node -pe "JSON.parse(require('fs').readFileSync('/dev/stdin').toString()).root")"
