...
export default function TabbedSynapseObjects(props: TabbedSynapseObjectsProps) {
  ...
  const [selectedTabConfig, setSelectedTabConfig] = useState<TabConfig>(tabConfigs[0])
  return <Box className="TabbedSynapseObjects">
    <Tabs
      value={selectedTabConfig.label}
      centered={centerTabs}
    >
      {tabConfigs.map((tabConfig, index) => {
        return <Tab key={index} value={tabConfig.label} disableRipple={true}
