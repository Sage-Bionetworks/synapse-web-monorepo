...
3. Prefer MUI components (`Typography`, `Button`, `Chip`, `TextField`, etc.) when an existing component is a reasonable match — even if a couple of `sx` overrides or style tweaks are needed. Staying within MUI keeps components consistently themed and easier to maintain. Only fall back to plain HTML (prefer an appropriate semantic element over `div` or `span`) when no MUI component is a reasonable fit.
...
6. When Figma outputs an image asset for what looks like an icon (a URL in a `const imgX = "..."` at the top of the generated code), always fetch the node screenshot with `get_screenshot` before implementing — the color is baked into the image and won't appear in the generated CSS. Use the screenshot to identify the icon's shape and color, then follow this decision tree:
   - **MUI icon matches closely** → use it from `@mui/icons-material`. Do not add new icon packages.
   - **No MUI icon matches** → the Figma MCP cannot export raw SVGs (all tools return rasters or expiring URLs). Ask the user to export the SVG manually from Figma (right-click the node → Export → SVG), then import it as a React component using the repo's existing `vite-plugin-svgr` support: `import MyIcon from './my-icon.svg?react'`.
   - **Never commit Figma asset URLs** — they expire after 7 days.
