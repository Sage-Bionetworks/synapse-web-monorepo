...
import useQueryWrapperData from '../../../src/components/QueryWrapper/useQueryWrapperData'
import {
  QueryBundleRequest,
  QueryResultBundle,
  ...
  describe('paginated query', () => {
    it('returns paginated data and all related controls', async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.isFetchingNextPage).toBeUndefined()
        expect(result.current.currentPage).toBeUndefined()
        expect(result.current.setCurrentPage).toBeUndefined()
      })
    })
