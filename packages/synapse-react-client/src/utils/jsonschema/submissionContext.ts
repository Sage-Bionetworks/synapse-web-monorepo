 ...
 *
 * To gate a property to a specific submission context, a client can use conditional logic in
 * the JSON Schema against the 'x-synapse-submissionContext' property.
 *
 * A property that should only appear for a RENEWAL (or only for a REQUEST) is declared inside
 * an `allOf` entry keyed on the reserved `x-synapse-submissionContext` property, e.g.:
 *
 * ```json
 * {
 *   "allOf": [{
 ...
 * context" select box, without hand-editing a JSON Schema.
 */
 ...
 * `then.properties`) are left alone.
 */
...
/**
 * Enumerate every leaf property the schema declares along with which submission
 * context applies to each. The reserved `x-synapse-submissionContext`
 ...
 * a property never leaves extra schema components behind.
 */
 ...
 *
 * Does not modify the property's requiredness behind at its old location. Callers that need to
 * preserve it (e.g. changing a property's context) should follow up with `setSchemaPropertyRequired`.
 */
 ...
 * requiredness. A no-op if the property doesn't resolve, or is already at that context.
 */
