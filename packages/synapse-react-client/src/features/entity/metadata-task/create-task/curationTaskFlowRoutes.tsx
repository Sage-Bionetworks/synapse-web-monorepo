 ...
 *
 * These are plain `RouteObject`s meant to be spliced directly into a host data router's own route
 * tree (e.g. under a `{ path: 'create', children: getCurationTaskFlowRoutes(...) }` entry -- see
 * `MetadataTasksPageRouter`), rather than rendered via a nested `<Routes>` inside a component. A
 * nested `<Routes>` mounted under an ancestor splat (`create/*`) route made relative navigation
 * (`navigate('../confirmation/...')`) resolve incorrectly -- it popped past the flow's own routes
 * and produced a URL the host router couldn't match, a 404. Splicing flat `RouteObject`s into the
 * host's own tree avoids the nested-router boundary that caused that.
 */
