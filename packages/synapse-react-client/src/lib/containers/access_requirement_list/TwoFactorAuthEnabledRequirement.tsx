...
function getState(
  ...
  if (!isSignedIn) {
    return RequirementItemState.LOCKED
  }
  if (isLoading) {
    return RequirementItemState.LOADING
  }
  return status === 'ENABLED'
    ? RequirementItemState.COMPLETE
    : RequirementItemState.LOCKED
}
...
export default function TwoFactorAuthEnabledRequirement() {
  ...
  return (
    <RequirementItem
      state={getState(isSignedIn, isLoading, twoFactorAuthStatus?.status)}
      actions={actions}
    >
