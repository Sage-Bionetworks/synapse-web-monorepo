...
describe('DataGridWebSocket', () => {
  ...
  describe('onPatchApplied callback', () => {
    /**
     * Build a model that has two columns and one row with initial cell values,
     * then flush all creation ops so the model is in a clean state ready for
     * delta patches.
     */
    function createModelWithRow(): GridModel {
      const model = Model.create(gridSchema)
      const colNamesVec = model.api.vec(['columnNames'])
      colNamesVec.set([
        [0, s.con('col1')],
        [1, s.con('col2')],
      ])
      // Remove any schema-initialised rows before inserting our own
      ...
      rowsArr.ins(0, [
        s.obj({
          data: s.vec(s.con('original1'), s.con('original2')),
          metadata: s.obj({}),
        }),
      ])
      model.api.flush()
    ...
    it('calls onPatchApplied with the correct authorSid and changed cells when an incoming patch modifies a cell value', () => {
      vi.spyOn(console, 'debug').mockImplementation(() => {})
      vi.spyOn(console, 'log').mockImplementation(() => {})
      ...
      // Source model: columns + row with initial data
      const sourceModel = createModelWithRow()

      // Give the ws an equivalent model (fork shares the same CRDT node IDs)
      // and seed rowsBaseline so diffRows has a before-snapshot to compare against
      const wsModel = (sourceModel as unknown as Model).fork(
        replicaId,
      ) as unknown as GridModel
      ws['model'] = wsModel
      ws['rowsBaseline'] = wsModel.api.getSnapshot()?.rows ?? null

      // Now change a cell in the source model to produce a delta patch
      sourceModel.api.vec(['rows', '0', 'data']).set([[0, s.con('changed1')]])
      const patch = sourceModel.api.flush()
      const encoded = encode(patch)
      const patchMessage = [4, 1, { type: 'patch', body: encoded }]

      mockSocket.simulateMessage(patchMessage)

      expect(onPatchApplied).toHaveBeenCalledTimes(1)
      const [, changes] = onPatchApplied.mock.calls[0] as [
        number,
        { rowIndex: number; colName: string }[],
      ]
      expect(changes).toContainEqual({ rowIndex: 0, colName: 'col1' })
      // col2 was not changed
      expect(changes).not.toContainEqual({ rowIndex: 0, colName: 'col2' })
    })

    it('does not call onPatchApplied when an incoming patch makes no row-data changes', () => {
      vi.spyOn(console, 'debug').mockImplementation(() => {})
      vi.spyOn(console, 'log').mockImplementation(() => {})
      ...
      // This patch only changes columnNames, not row data
      const patchMessage = buildPatchResponseMessage(sourceModel)
      mockSocket.simulateMessage(patchMessage)
    ...
    it('calls onPatchApplied with replicaId as the authorSid for local edits sent via sendPatch', () => {
      vi.useFakeTimers()
      vi.spyOn(console, 'debug').mockImplementation(() => {})
      vi.spyOn(console, 'log').mockImplementation(() => {})

      const onPatchApplied = vi.fn()
      const { ws } = createDataGridWebSocket({
        onPatchApplied,
        model: null,
      })

      // Set up a model with a row and seed rowsBaseline
      const baseModel = createModelWithRow()
      const wsModel = (baseModel as unknown as Model).fork(
        replicaId,
      ) as unknown as GridModel
      ws['model'] = wsModel
      ws['rowsBaseline'] = wsModel.api.getSnapshot()?.rows ?? null

      // Make a local edit directly on the ws model (simulating user typing)
      wsModel.api.vec(['rows', '0', 'data']).set([[1, s.con('local_edit')]])

      ws.sendPatch()
      vi.advanceTimersByTime(0)

      expect(onPatchApplied).toHaveBeenCalledTimes(1)
      const [authorSid, changes] = onPatchApplied.mock.calls[0] as [
        number,
        { rowIndex: number; colName: string }[],
      ]
      expect(authorSid).toBe(replicaId)
      expect(changes).toContainEqual({ rowIndex: 0, colName: 'col2' })
