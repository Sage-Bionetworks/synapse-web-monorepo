...
export type ClientErrorProps = {
  /** The error encountered on an API request to Synapse */
  error: SynapseClientError
  /** Component to display while waiting for session initialization (e.g. progressbar, spinner, or skeleton) */
  loadingIndicator: React.ReactNode
  /** Function to reload the component tree (provided by react-error-boundary) */
  reloadFn?: () => void
}

export default function ClientError(props: ClientErrorProps) {
  const { error, reloadFn, loadingIndicator } = props
  ...
  useEffect(
    function reloadOnSessionInitialized() {
      if (
  ...
  if (shouldAutoReloadWhenSessionInitializes.current) {
    // User is not signed in because the session is still being initialized.
    // Show a loading indicator. The above effect will trigger a reload when the session initialization is complete.
