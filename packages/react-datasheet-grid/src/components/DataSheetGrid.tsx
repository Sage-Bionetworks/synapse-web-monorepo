...
export const DataSheetGrid = React.memo(
  React.forwardRef<DataSheetGridRef, DataSheetGridProps<any>>(
    <T,>(
      ...
      const hasStickyLeftColumn =
        pinFirstColumn || Boolean(columns[1]?.stickyLeft)
        ...
        if (activeCell?.row !== undefined) {
          items.push(
            ...
            },
            {
              type: 'PASTE',
              action: (): void => {
                // Reading the clipboard is async, but a menu action returns void
                  ...
                  if (navigator.clipboard.read !== undefined) {
                    ...
                    for (const item of items) {
                      ...
                      if (item.types.includes('text/html')) {
                        ...
                        pasteData = parseTextPlainData(
                          await plainTextData.text(),
                        )
                      } else if (item.types.includes('text')) {
                        const htmlTextData = await item.getType('text')
                        pasteData = parseTextHtmlData(await htmlTextData.text())
                      }
                      await applyPasteDataToDatasheet(pasteData)
                    }
                  } else if (navigator.clipboard.readText !== undefined) {
                    const text = await navigator.clipboard.readText()
                    await applyPasteDataToDatasheet(parseTextPlainData(text))
                  } else {
                    alert(
